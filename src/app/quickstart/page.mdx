...
This guide will get you all set up and ready to use the Vatly API. We'll cover how to get started using one of our API clients and how to make your first API request. We'll also look at where to go next to find all the information you need to take full advantage of our powerful REST API. {{ className: 'lead' }}
...
```bash {{ language: 'js' }}
# Install the Vatly JavaScript SDK
npm install @example/protocol-api --save
```

```bash {{ language: 'python' }}
# Install the Vatly Python SDK
...
After picking your preferred client, you are ready to make your first call to the Vatly API. Below, you can see how to send a GET request to the Conversations endpoint to get a list of all your conversations. In the cURL example, results are limited to ten conversations, the default page length for each client.
...
- [Learn about the different error messages in Vatly](/errors)
