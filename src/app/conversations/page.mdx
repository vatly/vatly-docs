...
<Properties>
  ...
  <Property name="redirectUrlCanceled" type="string">
    The URL to which theuser should get redirected when the user cancels the checkout.
  </Property>
...
<Row>
  ...
  <Col sticky>
    ...
    curl https://api.vatly.com/v1/checkouts \
      -H "Authorization: Bearer {token}" \
      -d 'products[]'="subscription_d6ZPGVg4PEewUukwYd5zH"
...
## Retrieve a conversation {{ tag: 'GET', label: '/v1/checkouts/:id' }}
...
## Update a conversation {{ tag: 'PUT', label: '/v1/checkouts/:id' }}
...
<Row>
  ...
  <Col sticky>
    ...
    client.checkouts.update("xgQQXg3hrtjh7AvZ", is_muted=True)
    ```
...
## Delete a conversation {{ tag: 'DELETE', label: '/v1/checkouts/:id' }}

<Row>
  <Col>

    This endpoint allows you to delete your checkouts in Vatly. Note: This will permanently delete the conversation and all its messages — archive it instead if you want to be able to restore it later.
