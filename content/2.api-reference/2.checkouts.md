...
| `customerId` | `string \| null` | The customer associated with this checkout. Always present; `null` until a customer is associated — for an anonymous checkout this happens when the buyer completes payment. |
...
| `links` | `object` | HATEOAS links to related resources. Contains `checkoutUrl` (the hosted checkout page URL), `self`, and `order`. `order` is always present and is `null` until the checkout completes. |
