...
| `cancellationReason` | `string \| null` | Why the subscription was canceled. Can be `payment_failure` (payment recovery was exhausted after failed renewals), `merchant_request` (the merchant canceled the subscription), or `customer_request` (the customer canceled from the self-service portal). Null unless a cancellation has been requested. |
...
| `scheduledUpdate` | `object \| null` | The target values for a plan/price/quantity change scheduled to take effect at the subscription's next billing cycle, set by an [update](#update-a-subscription) with `applyImmediately: false`. The rest of the resource reflects the subscription's *current* state; `scheduledUpdate` is what it will become. Contains `subscriptionPlanId`, `name`, `description`, `basePrice`, `quantity`, `interval`, `intervalCount`, and `effectiveAt` (the next renewal date when the change applies, or `null` if there is no scheduled renewal). Null when nothing is pending. This is the authoritative way to reconcile a pending change — always present on the resource, so you don't need to rely on the `subscription.update_scheduled` webhook. Cleared when the change is applied at renewal, and when it is discarded because the subscription was canceled before that renewal. |
...
| `applyImmediately` | `boolean` | Whether to apply changes immediately or at the end of the current billing period. When `true`, changes take effect immediately (with proration if enabled). When `false`, the change is scheduled for the end of the current period and surfaced on the subscription's `scheduledUpdate`. Default: `false`. |
| `invoiceImmediately` | `boolean` | Whether to generate and charge an invoice immediately for proration. Only applies when `applyImmediately` and `prorate` are both true. When `false` (the default), the proration delta is parked on the current cycle and billed as a line on the next renewal invoice — the customer gets one invoice and one payment covering both. If the subscription is canceled before that renewal, the parked amount is waived, so a customer is never charged at the moment they cancel. **Set this to `true` on yearly and other long-interval plans**, where the deferred delta may not be billed for many months and is waived entirely if the customer cancels in between. Default: `false`. |
