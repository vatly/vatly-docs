---
title: "Idempotency"
description: "Learn how to use idempotency keys to safely retry API requests without creating duplicate resources."
---
...
For example, if you're creating a new subscription and the request fails due to a network error, you can safely retry the request without worrying about creating duplicate subscriptions.
...
```bash [cURL]
curl -X POST https://api.vatly.com/v1/subscriptions \
  -H "Authorization: Bearer live_your_api_key_here" \
  -H "Idempotency-Key: 123e4567-e89b-12d3-a456-426614174000" \
  -d subscription="Hello, World!"
```

```php
$vatly = new \Vatly\Client('your-token');

$vatly->subscriptions->create([
...
function createSubscription($vatly, $subscription) {
    ...
    while ($attempt < $maxRetries) {
        try {
            return $vatly->subscriptions->create([
