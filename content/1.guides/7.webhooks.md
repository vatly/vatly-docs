...
::note
The event fields are at the **top level** of the body — `id`, `resource`, `eventName`, `entityType`, `entityId`, `object`, `createdAt`, `testmode`, `links`. There is **no `data` wrapper**: read `body.eventName` and `body.object`, not `body.data.eventName`. The full `WebhookDelivery` schema is in the [OpenAPI spec](https://docs.vatly.com/openapi.yaml).
::
